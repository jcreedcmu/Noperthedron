...
This means:
- `native_decide` sees and evaluates the real 18.7M-row data
- The kernel sees the dummy values, but never uses them in proofs
  (all proof paths go through `native_decide` or sorry'd oracle fields)
...
witnesses above. `globalCert.data = globalCertData` and `localCert.data = localCertData`
hold by definition, so the bridge lemmas apply directly.
-/
