...
theorem rupert_imp_rupert_set (v : Finset ℝ³) :
  ...
  -- The inner shadow is the continuous image of a compact set, hence closed.
  have inner_shadow_closed : IsClosed inner_shadow := by
    have hf : Continuous fun p : ℝ³ => inner_offset + proj_xy (inner_rot.toEuclideanLin p) := by
      rw [proj_xy_eq_proj_xyL]
      exact continuous_const.add
        (proj_xyL.continuous.comp (LinearMap.continuous_of_finiteDimensional _))
    exact (((Finset.finite_toSet v).isCompact_convexHull ℝ).image hf).isClosed
