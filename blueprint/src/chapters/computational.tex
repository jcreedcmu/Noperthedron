...
\begin{proof}\uses{def:nopertQ}
By exhibiting the table and running the validity checking algorithm.
Formalized as \texttt{Noperthedron.Verified.Native.exists\_solution\_table}
in the build-on-demand \texttt{VerifiedNative} library (checked with
\texttt{native\_decide}; a kernel-only \texttt{VerifiedKernel} variant is in
progress), which is kept out of the default build targets so that CI runs
stay fast.
\end{proof}
...
\begin{proof}\leanok
\uses{thm:local_rational,lemma:nopert_verts_norm_le_one,lem:congruent,thm:nopertQ_approximate}
\end{proof}
