...
\begin{lemma} \label{lem:hullscalarprod}
    If $\mathcal{P} \subseteq \R^2$ is a convex polygon, $S \in \mathcal{P}^\circ$ and $w \in \R^2$ then
    \[
        \langle S ,w \rangle \leq \max_{P \in \PP} \langle P ,w\rangle.
    \]
\end{lemma}

\begin{proof}
See \cite{polyhedron.without.rupert}, Lemma 18.
\end{proof}
...
\begin{lemma} \label{lem:n2}
    ...
    If
    \(
        \left|\partial_{x_i}\partial_{x_j}f(x_1,\dots,x_n)\right| \leq 1
    \)
...
\begin{proof}
\uses{lem:hullscalarprod,lem:leq1,lem:n2}
