...
\lean{GlobalTheorem.rotation_partials_bounded, GlobalTheorem.rotation_third_partials_bounded}
\leanok
...
\leanok
    Let $f:\R^n\to \R$ be a $C^3$-function and $x_1,\dots,x_n,y_1,\dots,y_n \in \R$ such that $|x_1-y_1|,\dots,|x_n-y_n|\leq \varepsilon$.
    If
    \(
    ...
    \[
    |f(x)-f(y)|\leq \varepsilon \sum_{i=1}^n |\partial_{x_i} f(x)|
      + \frac{\varepsilon^2}{2} \sum_{i=1}^n \sum_{j=1}^n |\partial_{x_i}\partial_{x_j} f(x)|
      + \frac{n^3}{6}\varepsilon^3.
    \]
\end{lemma}
...
order: expand $g(t) = f((1-t)x + ty)$ to second order with Lagrange remainder,
bound $|g'(0)|$ by the first sum, $|g''(0)|/2$ by the second (the second
partials are evaluated exactly at $x$, not bounded), and $|g'''(c)| \le
n^3\varepsilon^3$ using the third-partial bound.
\end{proof}
...
\leanok
    ...
    \begin{align*}
        G \coloneqq{}& \langle R(\alphab) \Mib S,w \rangle - \epsilon\cdot\big(|\langle R'(\alphab)  \Mib S,w \rangle|+|\langle R(\alphab) \Mib^\theta S,w \rangle|+|\langle R(\alphab) \Mib^\phi S,w \rangle|\big)\\
        &- \frac{\epsilon^2}{2}\cdot\big(|\langle R(\alphab) \Mib S,w \rangle|
          + 2|\langle R'(\alphab) \Mib^\theta S,w \rangle| + 2|\langle R'(\alphab) \Mib^\phi S,w \rangle|\\
        &\qquad\quad + |\langle R(\alphab) \Mib^{\theta\theta} S,w \rangle| + 2|\langle R(\alphab) \Mib^{\theta\phi} S,w \rangle| + |\langle R(\alphab) \Mib^{\phi\phi} S,w \rangle|\big)
          - \frac{9\epsilon^3}{2},\\
        H_P \coloneqq{}& \langle \Miib P,w \rangle + \epsilon\cdot\big(|\langle \Miib^\theta P,w \rangle|+|\langle  \Miib^\varphi P,w \rangle|\big)\\
        &+ \frac{\epsilon^2}{2}\cdot\big(|\langle \Miib^{\theta\theta} P,w \rangle| + 2|\langle \Miib^{\theta\phi} P,w \rangle| + |\langle \Miib^{\phi\phi} P,w \rangle|\big) + \frac{4\epsilon^3}{3}, \quad \text{ for } P \in \PPP.
    \end{align*}
    ...
    (This is a second-order strengthening of \cite{polyhedron.without.rupert},
    Theorem 17, whose penalty is first-order with quadratic remainders
    $9\epsilon^2/2$ and $2\epsilon^2$: here the exact second partials at the
    center pose are charged at weight $\epsilon^2/2$ — with multiplicities from
    the symmetric Hessian table — and only the cubic Lagrange remainders
    $\frac{3^3}{6}\epsilon^3$ and $\frac{2^3}{6}\epsilon^3$ are bounded via
    Lemma~\ref{lem:leq1}.)
\end{theorem}
